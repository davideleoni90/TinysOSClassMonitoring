...
implementation {
	...
	*/
	...
	to a pc via serial port) thanks to the command "printf". Hence PrintfC has to be included (it wires to MainC so it's sufficient
	including it to get Printf stack started), as well as SerialStartC, which also is wired to MainC and so it ensures that when
	"booted" event is signalled, we can rely on the serial port 
	*/
	...
	*/
	...
	"SEND_QUEUE_DEPTH" in the header file Network.h): in this way, in case it is temporarely not possible to send a packet through
	the radio, it's not lost (unless the buffer queue is full) and will be sent later as soon as radio is available again
	*/
	...
	*/
	...
	/*
	This is for the mote to estimate the quality of the link to its neighbors
	*/
	...
	*/
	...
	/*
	We use a queue to temporarily store the data received from the accelerometer before sending them: this is very
