...
operating system designed for sensor networks, which excutes on the motes the code writenn by developers in a very
efficient way. Sensor networks are in fact made of nodes with limited hardware resources
...
processing the data collected)</li>
</ol>
...
to be delivered to the root mote with the shortest number of hops, i.e. minimizing the number of forwarders in the path from a producer to the root
node. The protocol uses a parameter called "ETX" (expected number of transmissions) as a gradient, so it always chooses the path from a producer to
...
of TinyOs; they also make use of the same method (4bitLinkEstimation) to estimate the quality of the links with the neighbor nodes, and this determines how each
...
messages in a queue before actually sending them: the proposed solution includes a further queue, so we end up with a two-levels queue. This comes in handy when
...
out to forward the data. The payload of messages created by producers, and forwarded by forwarders, not only contains the measure of acceleration sampled, but also
...
the code running on the root mote also includes the default TinyOs implementation of the serial communication stack, because it is necessary to communicate
the data received from the network to the host using its serial port</li>
</ul>
</p>
...
<h3>Software implementation</h3>
<p align="justify">
The release of TinyOs contains an application named "Mviz" with the aim of showing how it's possibile to implement the Collection Tree Protocol (CTP): when
...
them, "mviz", is the java counterpart of "Mviz" and is specifically designed to give a graphical representation of the nodes and links playing in
...
<li>mviz is meant to interact with a network where all the motes feature a sensor, so all of them are supposed to be the source of at least one
...
to track forwarders (not only producers), but the problem is solved in the customized version thanks to the fact that all motes append their ID to the payload of every
...
<li><i>hopcount</i>: since the maximum size for the two above arrays has to be defined in advance (see header "Network.h"), every time a mote
handles a message it increments this field, which is in fact a counter, so then it's possible to understand the exact number of nodes in the
path</li>
</ol>
Combining these three fields, for each message it is possible to get the list of motes and links it passed through, together with the quality of all the
links themselves. The main features of the java application are now presented.
<h3>Links Viewer</h3>
<p>
A table containing a brief description of all the links in the network: the ID of the two motes connected by the link and its quality. The last
value corresponds to the probability that a message sent by one vertex of the link is received by the other vertex and it's computed as a function of
link quality estimated by both the involved motes. As a matter of fact, if a message is sent from nodes A to B and other from B to A, the table creates a row for
the two links, but they carry the same value of "Quality", updated to the last message that crossed that link.
</p>
<p align="center">
<br><br>
